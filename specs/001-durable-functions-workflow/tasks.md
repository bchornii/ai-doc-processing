...
- [X] T003 [P] Update Directory.Packages.props with all required NuGet package versions: Microsoft.Azure.Functions.Worker 2.x, Microsoft.Azure.Functions.Worker.Sdk 2.x, Microsoft.Azure.Functions.Worker.Extensions.Http 3.x, Microsoft.Azure.Functions.Worker.Extensions.Http.AspNetCore 1.x, Microsoft.Azure.Functions.Worker.Extensions.DurableTask 1.x, Microsoft.Azure.Functions.Worker.Extensions.DurableTask.AzureManaged latest, Microsoft.Azure.Functions.Worker.Extensions.Storage.Queues 6.x, Azure.Monitor.OpenTelemetry.Exporter latest, Microsoft.Azure.Functions.Worker.OpenTelemetry latest, Azure.Identity 1.x, xunit latest, xunit.runner.visualstudio latest, Microsoft.NET.Test.Sdk latest, NSubstitute latest, coverlet.collector latest
...
- [X] T008 Create src/DocumentProcessing.Functions/Program.cs — configure isolated worker host via FunctionsApplication.CreateBuilder + ConfigureFunctionsWebApplication(); configure OpenTelemetry telemetry via AddOpenTelemetry().UseFunctionsWorkerDefaults().UseAzureMonitorExporter(); note: DTS backend (storageProvider type=azureManaged, connectionStringName=DTS_CONNECTION_STRING, hubName=%TASKHUB_NAME%) is configured in host.json extensions.durableTask, not in Program.cs; leave placeholder comments for DI activity registrations (to be filled in Phase 2 T036)
- [X] T009 [P] Create src/DocumentProcessing.Functions/host.json — set version 2.0, telemetryMode OpenTelemetry; configure extensions.durableTask.storageProvider with type=azureManaged and connectionStringName=DTS_CONNECTION_STRING; configure extensions.durableTask.hubName=%TASKHUB_NAME%; set logging:logLevel:Default to Information (suppress Azure.Core and Azure.Identity to Warning); note: Durable Functions activity retry policies are configured per-call in orchestrator code via TaskOptions (see T040, T048, T050), not in host.json
...
- [X] T041 [P] [US3] Create src/DocumentProcessing.Functions/Activities/ClassifyDocumentActivity.cs — instance class with IClassifyDocumentActivity injected via constructor; RunAsync method decorated with [Function(nameof(ClassifyDocumentActivity))]; method accepts [ActivityTrigger] composite input record (ContainerName, BlobName, CorrelationId) and FunctionContext executionContext; returns Task<ConfidenceResult<Classifications>>; log start/complete with structured correlationId and instanceId
- [X] T042 [P] [US3] Create src/DocumentProcessing.Functions/Activities/PersistResultActivity.cs — instance class with IPersistResultActivity injected via constructor; RunAsync method decorated with [Function(nameof(PersistResultActivity))]; method accepts [ActivityTrigger] composite input record (ContainerName, BlobName, Content as byte[], CorrelationId) and FunctionContext executionContext; returns Task<bool>; orchestration caller must serialize the result to UTF-8 byte[] before passing; log start/complete with structured properties
- [X] T043 [P] [US3] Create src/DocumentProcessing.Functions/Activities/ExtractInvoiceActivity.cs — instance class with IExtractInvoiceActivity injected via constructor; RunAsync method decorated with [Function(nameof(ExtractInvoiceActivity))]; method accepts [ActivityTrigger] composite input record (ContainerName, BlobName, PageStart, PageEnd, CorrelationId) and FunctionContext executionContext; returns Task<ConfidenceResult<Invoice>>; log structured properties
- [X] T044 [P] [US3] Create src/DocumentProcessing.Functions/Activities/ValidateInvoiceActivity.cs — instance class with IValidateInvoiceActivity injected via constructor; RunAsync method decorated with [Function(nameof(ValidateInvoiceActivity))]; method accepts [ActivityTrigger] composite input record (InvoiceName, Data as Invoice, CorrelationId) and FunctionContext executionContext; returns Task<ValidationResult>; log structured properties
- [X] T045 [P] [US3] Create src/DocumentProcessing.Functions/Activities/ExtractContractActivity.cs — instance class with IExtractContractActivity injected via constructor; RunAsync decorated with [Function(nameof(ExtractContractActivity))]; accepts [ActivityTrigger] composite input (ContainerName, BlobName, PageStart, PageEnd, CorrelationId) and FunctionContext; returns Task<ConfidenceResult<ContractData>>; log structured properties; and src/DocumentProcessing.Functions/Activities/ValidateContractActivity.cs — instance class with IValidateContractActivity injected via constructor; RunAsync decorated with [Function(nameof(ValidateContractActivity))]; accepts [ActivityTrigger] composite input (ContractName, Data as ContractData, CorrelationId) and FunctionContext; returns Task<ValidationResult>; log structured properties
- [X] T046 [P] [US3] Create src/DocumentProcessing.Functions/Activities/DetectBoundariesActivity.cs — instance class with IDetectBoundariesActivity injected via constructor; RunAsync decorated with [Function(nameof(DetectBoundariesActivity))]; accepts [ActivityTrigger] composite input (ContainerName, BlobName, CorrelationId) and FunctionContext; returns Task<BoundaryDetectionResult>; log structured properties; and src/DocumentProcessing.Functions/Activities/ExtractGeneralDocumentActivity.cs — instance class with IExtractGeneralDocumentActivity injected via constructor; RunAsync decorated with [Function(nameof(ExtractGeneralDocumentActivity))]; accepts [ActivityTrigger] composite input (ContainerName, BlobName, PageStart, PageEnd, CorrelationId) and FunctionContext; returns Task<ConfidenceResult<GeneralDocumentData>>; log structured properties
- [X] T047 [US3] Create src/DocumentProcessing.Functions/Orchestrations/ProcessDocumentSegmentWorkflow.cs — static class with a static RunAsync method decorated with [Function(nameof(ProcessDocumentSegmentWorkflow))]; method accepts [OrchestrationTrigger] TaskOrchestrationContext context; retrieve input via context.GetInput<DocumentSegmentInput>(); stub body returns WorkflowResult.Empty; use context.CreateReplaySafeLogger(); enables ProcessDocumentWorkflow BoundedDocument branch to compile; full implementation in Phase 7 (T050)
- [X] T048 [US3] Replace ProcessDocumentWorkflow stub body (T068) in src/DocumentProcessing.Functions/Orchestrations/ProcessDocumentWorkflow.cs — static class with a static RunAsync method decorated with [Function(nameof(ProcessDocumentWorkflow))]; method accepts [OrchestrationTrigger] TaskOrchestrationContext context; retrieve input via context.GetInput<ProcessDocumentFolderInput>(); for each documentFileName in input.Folder.DocumentFileNames: (1) ClassifyDocument → PersistResult (classification JSON as UTF-8 byte[]); (2) if result is null: log error, add error to WorkflowResult, continue; (3) if OverallConfidence < DocumentProcessingConstants.ConfidenceThreshold: log skip, add error entry, continue (FR-004); (4) route by first matching PageClassification in result.Data.PageClassifications — check in priority order Invoice, Contract, BoundedDocument, General, Email, None — and execute only the chain for the first match (first-match priority-order semantics per FR-004); use TaskOptions(new RetryPolicy(maxNumberOfAttempts: 3, firstRetryInterval: TimeSpan.FromSeconds(5), backoffCoefficient: 2.0)) on every CallActivityAsync call: Invoice→ExtractInvoice+PersistResult+ValidateInvoice+PersistResult; Contract→ExtractContract+PersistResult+ValidateContract+PersistResult; BoundedDocument→DetectBoundaries; if Segments.Count==0 add error and continue; else fan-out CallSubOrchestrationAsync(ProcessDocumentSegmentWorkflow, new DocumentSegmentInput(segment, folder.ContainerName, folder.Name, blobName, correlationId)) per segment+Task.WhenAll fan-in+PersistResult; General→ExtractGeneralDocument+PersistResult; Email/None→log and continue without error; aggregate and return WorkflowResult; use context.CreateReplaySafeLogger() throughout; MUST NOT read DateTime.UtcNow, Guid.NewGuid, or any I/O inline (FR-007)
